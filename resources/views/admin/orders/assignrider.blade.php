                ...
                <div class="card mb-4">
                 ...
                 <option value="{{$rider->id}}"> {{$rider->name}}</option>
                  ...
                  @endforeach
                      ...
                      @php
                      $pr = App\Models\Product::where('id',$salecenterorder->product_id)->first();
                      @endphp
                         
                         <input type="hidden" name="orderid" value="{{$name}}" />
                            <input type="hidden" name="cash" value="0" />
                              <input type="hidden" name="address" value="{{$salecenter->address}} " />
                                <input type="hidden" name="productname" value="{{$pr->name}}" />
                                <input type="hidden" name="quantity" value="{{$salecenterorder->quantity}}" />
                                <input type="hidden" name="color" value="{{$salecenterorder->colour_id}}" />
                                <input type="hidden" name="size" value="{{$salecenterorder->size_id}}" />
