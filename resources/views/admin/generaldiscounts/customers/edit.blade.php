...
@section('content')
    <input type="hidden" value="{{$activePage = 'generaldiscountIndex', $title = 'Edit Customer Discount - Nafia Garments'}}">
    <div class="main-content">
        <div class="breadcrumb">
            <h1>Customer Discount</h1>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="card-title mb-3">Edit Customer Discount</div>
                        <form class="forms-sample" method="POST" action="{{ route('customer_discount.update',$generaldiscount) }}">
                            @csrf()
                            @method('PUT')
                            <div class="row">
                                ...
                                <div class="col-md-6 form-group mb-3">
                                    ...
                                    <select class="form-control js-example-basic-single @error('customer_id') is-invalid @enderror" id="customer" name="customer_id">
                                        <option selected disabled> Select Customer </option>
                                        @foreach($customers as $customer)
                                            <option {{ $generaldiscount->customer_id == $customer->id ? 'selected':'' }} value="{{ $customer->id }}"> {{ $customer->name }} </option>
                                        @endforeach
                                    </select>
                                    @error('customer_id')
